...
const TaskDetail = ({ navigation, route }: any) => {
  ...
  const handleGetTime = () =>{
    ...
    const time = `${timeStart.getHours().toString().padStart(2, '0')}:${timeStart.getMinutes().toString().padStart(2, '0')} - ${timeEnd.getHours().toString().padStart(2, '0')}:${timeEnd.getMinutes().toString().padStart(2, '0')}`
    return time
  }
    ...
    taskDetail && (
      <View style={{ flex: 1 , backgroundColor:colors.bgColor}}>
        ...
        >
          ...
          <RowComponent >
            ...
            <RowComponent styles={{ flex: 1 }}>
              ...
              <SpaceComponent width={8} />
              <TextComponent text={`${HandleDateTime.ChangeMonth(taskDetail.dueDate.toDate().getMonth())} ${taskDetail.dueDate.toDate().getDate()}`} size={14} />
            </RowComponent> 
         ...
         {/* Description    */}
         <ScrollView>
        <SectionComponent styles={{paddingHorizontal:20, paddingVertical:14}}>
        ...
        <SpaceComponent height={20}/>
          ...
          <SectionComponent>
            <RowComponent justify="flex-start">
          <MaterialCommunityIcons name="circle-slice-8" size={24} color={colors.green} />
          ...
          <SectionComponent>
            ...
            {Array.from({length:3}).map((item,index)=> (
              <CardComponent key={`subtask ${index}`}
              styles={{
                marginTop:10
              }}
              >
